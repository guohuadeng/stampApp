...
class Smartwave_Blog_Model_Api extends Mage_Core_Model_Abstract
{
    ...
    public function getPostShortContent($post, $storeId = 0)
    {
        ...
        $isUseShortContent = Mage::getStoreConfig(Smartwave_Blog_Helper_Config::XML_BLOG_USESHORTCONTENT, $storeId);
        if ($isUseShortContent && trim($post->getShortContent())) {
            $content = trim($post->getShortContent());
        } elseif ((int)Mage::getStoreConfig(Smartwave_Blog_Helper_Config::XML_BLOG_READMORE, $storeId)) {
            $strManager = new Smartwave_Blog_Helper_Substring(
                array(
                     'input' => Mage::helper('blog')->filterWYS($post->getPostContent())
                )
            );
            $content = $strManager->getHtmlSubstr((int)Mage::getStoreConfig(Smartwave_Blog_Helper_Config::XML_BLOG_READMORE));
        }
        return $content;
    }
}